...
import (
	...
	"github.com/sandeep-jaiswar/engineering/pkg/ent/migrate"
...
// QueryRoles queries the roles edge of a Policies.
func (c *PoliciesClient) QueryRoles(po *Policies) *RoleQuery {
	query := (&RoleClient{config: c.config}).Query()
	query.path = func(context.Context) (fromV *sql.Selector, _ error) {
		id := po.ID
		step := sqlgraph.NewStep(
...
// QueryUsers queries the users edge of a Policies.
func (c *PoliciesClient) QueryUsers(po *Policies) *UserQuery {
...
type (
	hooks struct {
		Permission, Policies, Role, User []ent.Hook
	}
	inters struct {
		Permission, Policies, Role, User []ent.Interceptor
	}
)
