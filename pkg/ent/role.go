...
func (*Role) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		...
		case role.ForeignKeys[0]: // policies_roles
			values[i] = new(sql.NullInt64)
		default:
	...
	for i := range columns {
		...
		case role.ForeignKeys[0]:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				return fmt.Errorf("unexpected type %T for edge-field policies_roles", value)
			} else if value.Valid {
				r.policies_roles = new(int)
				*r.policies_roles = int(value.Int64)
			}
		default:
