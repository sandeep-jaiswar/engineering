...
func (*User) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		...
		case user.FieldUsername, user.FieldEmail, user.FieldHashedPassword, user.FieldStatus:
			values[i] = new(sql.NullString)
		case user.FieldCreatedAt, user.FieldUpdatedAt:
			values[i] = new(sql.NullTime)
		case user.ForeignKeys[0]: // policies_users
			values[i] = new(sql.NullInt64)
		default:
	...
	for i := range columns {
		...
		case user.ForeignKeys[0]:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				return fmt.Errorf("unexpected type %T for edge-field policies_users", value)
			} else if value.Valid {
				u.policies_users = new(int)
				*u.policies_users = int(value.Int64)
			}
		default:
